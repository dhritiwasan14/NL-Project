...
def getPredictedDistribution(v, w, wq, b):
    ### TO IMPLEMENT ###
    ...
    # print(b)
    # propogate values to hidden units
    prop_vals = visibleToHiddenVec(v, w, b)
    sampledHidden = sample(prop_vals)
    backProp = hiddenToVisible(sampledHidden, np.array([wq]))
    # print(backProp)
    return backProp[0, :]
...
def predictRatingMax(ratingDistribution):
    ...
    rating = np.argmax(ratingDistribution) + 1
    return rating
...
def predictRatingMean(ratingDistribution):
    ...
    norm_dist = ratingDistribution/ratingDistribution.sum(0)
    norm_dist = [norm_dist[i-1]*i for i in range(1, K+1)]
    rating = np.sum(norm_dist)
    return rating
...
def predictRatingExp(ratingDistribution):
    ...
    probs = softmax(ratingDistribution)
    norm_dist = probs/probs.sum(0)
    norm_dist = [norm_dist[i-1]*i for i in range(1, K+1)]
    return np.sum(norm_dist)

def predictMovieForUser(q, user, W, training, predictType="mean", b=None):
    # movie is movie idx
    # user is user ID
    # type can be "max" or "exp"
    ratingsForUser = lib.getRatingsForUser(user, training)
    v = getV(ratingsForUser)
    ratingDistribution = getPredictedDistribution(v, W[ratingsForUser[:, 0], :, :], W[q, :, :], b=b)
    if predictType == "max":
...
def predict(movies, users, W, training, predictType="mean", b=None):
    # given a list of movies and users, predict the rating for each (movie, user) pair
    # used to compute RMSE
    return [predictMovieForUser(movie, user, W, training, predictType=predictType, b=b) for (movie, user) in zip(movies, users)]

def predictForUser(user, W, training, predictType="mean", b=None):
    ### TO IMPLEMENT
    # given a user ID, predicts all movie ratings for the user
    predictions = np.zeros((W.shape[0]))
    for i in range(W.shape[0]):
        predictions[i] = predictMovieForUser(i, user, W, training, b=b)
